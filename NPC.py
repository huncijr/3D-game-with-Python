...
class NPC(AnimatedSprite):
    def __init__(self,game,path='C:/Users/hunor/Desktop/3D modellezes Project/3D game/NPC/1. ellenseg/0.png',pos=(9.5,5.5)
                 ,scale=0.9,shift=-0.1,animation_time=100): #scale=0.6 → a kép 60%-os méretű lesz az eredetihez,shift= a sprite függőleges eltolása
        super().__init__(game,path,pos,scale,shift,animation_time)
        self.attack_images = self.get_images(self.path + '/tamadas')
        self.death_images = self.get_images(self.path + '/meghalas')
        self.walk_images = self.get_images(self.path + '/jaras')
        self.pain_images = self.get_images(self.path + '/sebzes')
        self.idle_images = self.get_images(self.path + '/kulonbozo_nezet')
        self.attack_dist = randint(3,5)
        self.speed = 0.01
        self.size = 10
        self.health = 120
    ...
    def ray_cast_player_npc(self):
        ...
        for ray in range(NUM_RAYS):  # <-- MOST már a függvény része
            ...
            for i in range(MAX_DEPTH):
                ...
                if tile_horizontal == self.map_pos:
                    player_dist_h = depth_horizontal
                    break
                if tile_horizontal in self.game.map.world_map:
                    wall_dist_h = depth_horizontal
                    break
                x_hor += dx
                y_hor += dy
            ...
            depth_vertical = (x_vert - ox) / cos_a
            y_vert = oy + depth_vertical * sin_a
            ...
            for i in range(MAX_DEPTH):
                ...
                if tile_vert == self.map_pos:
                    player_dist_v = depth_vertical
                    break
                if tile_vert in self.game.map.world_map:
                    wall_dist_v = depth_vertical
                    break
                x_vert += dx
                y_vert += dy
